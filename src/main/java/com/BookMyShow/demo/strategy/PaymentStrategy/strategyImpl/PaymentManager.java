...
public class PaymentManager {
    ...
    public void pay(PaymentType paymentType, double amount) throws Exception {
        PaymentStrategy strategy = strategies.get(paymentType.name());
        if (strategy == null) {
            throw new IllegalArgumentException("Unsupported Pyment type: " + paymentType);
        }

        strategy.pay(strategy,amount);
    }
